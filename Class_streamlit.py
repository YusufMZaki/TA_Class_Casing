...
with Tab_Data: 
    with st.form("my_Data"):
        ...
        # Tab
        Tab_Variabel, Tab_Subs, Tab_Cement = st.tabs(["Casing Variable", "Subsurface Variable", "Mud"])
...
with Tab_Variabel: 
    ...
    if Catalog_select == "Manual":
        Manual_data_ = Custom_Casing.OD_df(Casing.OD)
        if len(Manual_data_.dropna()) == 0: Manual_data = Casing_data[Casing_data['1_Size Outside Diameter in. D'] == Casing.OD].iloc[-1:,:].reset_index(drop=True)
        else: Manual_data = Custom_Casing.OD_df_concat(Casing_data, Manual_data_.dropna())
...
with Tab_Design:
    form = st.form("my_Design")
    form.form_submit_button("Re-Calculate", use_container_width=True)
    with form: Tab_Burst_Collapse, Tab_Tension_Biaxial, Tab_Result = st.tabs(["Burst - Collapse", "Tension - Biaxial", "Casing Design"])
